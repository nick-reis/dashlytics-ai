...
export function useCustomers(id?: string) {
  ...
  async function removeCustomers(ids: string[]) {
    ...
    try {
      ...
      console.error("Failed to remove customer(s) ", err);
      setError(err.message || "Something went wrong");
    } finally {
      setLoading(false);
    }
  ...
  async function createCustomer(customer: CustomerSchema) {
    setLoading(true);
    setError(null);
    try {
  ...
  async function editCustomer(customerId: string, customer: CustomerSchema) {
    setLoading(true);
    setError(null);
    try {
  ...
  const refetch = useCallback(
    () => fetchCustomers({ showInitialLoading: true }),
    [fetchCustomers]
  );
